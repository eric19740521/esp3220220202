...
String bin = "/AppUpdate/ota-02.ino.esp32.bin"; // bin file name with a slash in front.
...
void execOTA() {
  ...
  // check contentLength and content type 前面程式碼解析出 內容長度 跟 內容是否有效
  if (contentLength && isValidContentType) {
    ...
    if (canBegin) {
      ...
      if (written == contentLength) {   //內容長度是否正確???
